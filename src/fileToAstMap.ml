...
let createTable  = 
   ...
   Hashtbl.add fileToAstMap "exp1.k-opt" {ast = [Function(Id "foo",[],[Dec (Id "a",Int 9);Dec(Id "b",Int 6)],[Assign(Id "a", Int 3); Assign(Id "b",Int 2);Int 5])];};
      ...
      Hashtbl.add fileToAstMap "exp0.k" {ast = [Function(Id "foo",[],[Dec(Id "a",Plus(Int 6,Int 3));Dec(Id "b",Plus (Int 4, Int 2))],[Assign(Id "a", Int 3); Assign(Id "b",Int 2);Plus(IdExp(Id "a"), IdExp(Id "b")); Assign (Id "b", Int 7)])];};
        ...
        Hashtbl.add fileToAstMap "fun3.k-opt" {ast = [];};
        Hashtbl.add fileToAstMap "fun4.k" {ast = [];};
        Hashtbl.add fileToAstMap "fun4.k-opt" {ast = [];};
        Hashtbl.add fileToAstMap "fun5.k" {ast = [];};
        Hashtbl.add fileToAstMap "fun5.k-opt" {ast = [];};
        Hashtbl.add fileToAstMap "fun6.k" {ast = [];};
        Hashtbl.add fileToAstMap "fun6.k-opt" {ast = [];};
        Hashtbl.add fileToAstMap "fun7.k" {ast = [];};
        Hashtbl.add fileToAstMap "fun7.k-opt" {ast = [];};
        Hashtbl.add fileToAstMap "fun8.k" {ast = [];};
        Hashtbl.add fileToAstMap "fun8.k-opt" {ast = [];};
        Hashtbl.add fileToAstMap "fun9.k" {ast = [];};
        Hashtbl.add fileToAstMap "fun9.k-opt" {ast = [];};
